/*********************************************************
CopyRight (C), 2013-2016,翔迅科技有限责任公司.
文件名: can.c
作者:千佳琦
创建日期: 2019-12-23
版本:
...
uint8           g_plc_data1[2 * DIO_CNT] = {0};              // 回采的PLC数据 add by 2023331
...
static uint32   g_permit_card_lost_frame[2][DIO_CNT] = {{0}};   //允许在规定的时间内板卡丢帧计数器
static uint32    count_1s = 0;   //定时清板卡丢帧计数器
                        ...
                        if (MCU_BACKUP != g_mcu_status)
                        {
                            ...
                            {								
								if ((g_mcu_heart_other[0] != 0xFFFF) && (g_mcu_heart_other[1] != 0xFFFF))
								{
									...
									{
										g_mcu_status = MCU_BACKUP;
										debug_line("MCU_MASTER! 4 recv_mcu\r\n");
									}
									else if (0x03 == get_mcu_Id())
									{
										g_mcu_status = MCU_MASTER;
										debug_line("MCU_MASTER! 3 recv_mcu\r\n");
									}
								}
                        ...
                        if (MCU_MASTER != g_mcu_status)
                        {
                            ...
                            {									
								if ((g_mcu_heart_other[0] != 0xFFFF) && (g_mcu_heart_other[1] != 0xFFFF))
								{
									...
									{
										g_mcu_status = MCU_BACKUP;
										debug_line("MCU_BACKUP! 4 recv_mcu\r\n");
									}
									else if (0x03 == get_mcu_Id())
									{
										g_mcu_status = MCU_MASTER;
										debug_line("MCU_BACKUP! 3 recv_mcu\r\n");
									}
								}								
                            }
                        }
								...
								if (1)	
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (recv_frame.data[4] != p_can_buff->DIO_status[card_num].data[offset].send_data[3])
                                        {
                                            ...
                                            //保存故障
											sprintf(str, "%2d %d", card_num, offset);
											save_log(1, __LINE__, str);
                    ...
                    if (4 == recv_frame.can_dlc)
                    {
                        if (recv_frame.data[recv_frame.can_dlc - 1] == calc_crc16_8(recv_frame.data, recv_frame.can_dlc - 1))   // 帧校验正确
                        {
                            ...
                            if (CAN_LINE_TRUST == g_can_line_status[can_line])
                            {
                                ...
                                if (0 != result)    // 帧数据正确
								{
									//在日志中记录发送的DO数据与回采的DO数据
									sprintf(str, "%d %d:%x %x %x:%x %x %x",card_num,can_line, p_can_buff->DIO_DO[card_num].data[offset].send_data[0], p_can_buff->DIO_DO[card_num].data[offset].send_data[1],p_can_buff->DIO_DO[card_num].data[offset].send_data[2], recv_frame.data[0], recv_frame.data[1], recv_frame.data[2]);
									save_log(1, __LINE__, str);
								}
								// 时间间隔
								...
								if (1)
								{
									...
									{
										g_plc_data1[2 * (card_num / 2)] = recv_frame.data[1];
										g_plc_data1[2 * (card_num / 2) + 1] = recv_frame.data[2];  //回采DO数据 
									}
								}
								else
								{
                        ...
                        if (recv_frame.data[recv_frame.can_dlc - 1] == calc_crc16_8(recv_frame.data, recv_frame.can_dlc - 1))   // 帧校验正确
                        {
                            ...
                            if (CAN_LINE_TRUST == g_can_line_status[can_line])
                            {
                                ...
                                if (0 == result)    // 帧数据正确
                                {
                                    ...
                                    {
                                        //保存故障
										sprintf(str, "%d %dms",offset, time_after);
										save_log(1, __LINE__, str);
                                    }
                                ...
                                {
                                    // 帧数据错误
									sprintf(str, "%d %dms %d %d",offset, time_after, recv_frame.data, p_can_buff->MVB_run.data[offset].send_data);
									save_log(1, __LINE__, str);
                                    return -1;
                                }
                            }
...
int can_recv_frame(struct can_frame recv_frame, T_can_buff *p_can_buff, uint8 can_line, struct timeval recv_tv)
{
    ...
    switch (can_id.hard_addr)
    {
        ...
        case 0x00:
        {
			...
			else if ((0x0500 == can_id.all) && (0x5A == recv_frame.data[0]) && (4 == recv_frame.can_dlc))
            {
                // 记录板卡自己宕机原因
                sprintf(str, "can%d recv card [0x%02X] line [%04d]", can_line, recv_frame.data[1], ((uint16)(recv_frame.data[2]) << 8) | (recv_frame.data[3]));
				save_log(1, __LINE__, str);
...
int can_send_frame(struct can_frame send_frame, int can_fd, T_can_buff *p_can_buff, T_can_data *p_can_data, uint8 can_line)
{
    ...
    while (sizeof(struct can_frame) != write(can_fd, &send_frame, sizeof(struct can_frame)))
    {
        ...
        {
            //保存故障
			sprintf(str, "0x%04X %d", send_frame.can_id, can_line);
			save_log(1, __LINE__, str);
...
/***************************add 3.13********************************/	
	for(i=0;i<10;i++)
	{
		//当板卡一帧超时后，状态变为err，连续对板卡状态进行判断20次，若20次期间板卡状态还是err，则认为是真故障，进入到elseif分支将真实的状态信息发给MVB，若在判断6次期间板卡的状态切换为ok，则进入else分支发送板卡正常状态，在判断6次期间若板卡状态是err，则将所有板卡设置为正常状态发送给MVB，本方案的目的是解决闪报故障
		if((check[i] < 20) && (DIO_FAKE_ERR == g_dio_status[i]))
		{
			check[i]++;
			if((i >= 0) && (i < 8))
			{
				CLR_BIT(send_frame.data[1], i);
			}
			else if(i >= 8)
			{
				CLR_BIT(send_frame.data[2], i-8);
			}
			
			sprintf(str, "%d", check[i]);
			save_log(1, __LINE__, str);
		...
		else if(check[i] >= 20)
		{
			if((i >= 0) && (i < 8))
			{
			    ((DIO_FAKE_ERR == g_dio_status[i]) || (DIO_TRUE_ERR == g_dio_status[i]))  ? SET_BIT(send_frame.data[1], i) : CLR_BIT(send_frame.data[1], i);  // DIO1  0 正常, 1 故障
			}
			else if(i >= 8)
			{				
				((DIO_FAKE_ERR == g_dio_status[i]) || (DIO_TRUE_ERR == g_dio_status[i]))  ? SET_BIT(send_frame.data[2], i-8) : CLR_BIT(send_frame.data[2], i-8);  // DIO1  0 正常, 1 故障
			}
...
int can_periodic(int *p_can_fd, T_can_buff **p_can_buff)
{
    ...
    if ((mcu_heart_other_err[0] >= MCU_MAX_ERR_CNT) && (mcu_heart_other_err[1] >= MCU_MAX_ERR_CNT))  // nS未收到心跳
    {
        ...
        if ((CAN_LINE_TRUST == g_can_line_status[0]) || (CAN_LINE_TRUST == g_can_line_status[1]))
        {
            ...
            if (MCU_MASTER != g_mcu_status)
            {
                debug_line("MCU_MASTER! 12 mcu_heart\r\n");
				
				// sprintf(str, "CAN_LINE 12 MCU_HEART!");
				// save_log(1, __LINE__, str);
                // 当前板卡设为主设备
                g_mcu_status = MCU_MASTER;
            }
    ...
    else if ((mcu_heart_other_err[0] >= MCU_MAX_ERR_CNT) && (mcu_heart_other_err[1] <= 2))
    {
        ...
        if (CAN_LINE_ERR != g_can_line_status[0])
        {
            ...
            {			
				//备板不将其线路设置为故障，主板将其线路设置为故障
                if (g_mcu_status == MCU_BACKUP)
				{
					g_can_line_status[0] = CAN_LINE_OK;
					g_can_line_status[1] = CAN_LINE_TRUST;
				}
    ...
    if ((g_mvb_err_cnt[0] >= MVB_MAX_ERR_CNT) && (g_mvb_err_cnt[1] >= MVB_MAX_ERR_CNT))
    {
        ...
        {
            //debug_line("MVB ERR!\r\n");
			sprintf(str, "%d %d", g_mvb_err_cnt[0], g_mvb_err_cnt[1]);
			save_log(1, __LINE__, str);
            ...
            for (offset = 0; offset < CAN_FRAME_CNT; offset++)
            {
                ...
                if (1 == p_can_buff[can_line]->DIO_status[card_num].data[offset].frame_send_flag)
                {
                    ...
                    else
                    {
                        ...
                        if (time_after > FRAME_TIMEOUT)
                        {											
							g_permit_card_lost_frame[can_line][card_num]++;
							p_can_buff[can_line]->DIO_status[card_num].data[offset].frame_send_flag = 0;
							p_can_buff[can_line]->DIO_status[card_num].data[offset].frame_recv_flag = 0;
							
							sprintf(str, "%d %d %d %d", can_line, offset, card_num, g_permit_card_lost_frame[can_line][card_num]);
							save_log(1, __LINE__, str);
							//同一块板卡丢失不同序列号的帧后，累加到15次进行板卡切换
							if((g_permit_card_lost_frame[can_line][card_num] < 30) && (count_1s >= 50))
							{
								count_1s = 0;   //1s内计时清零，重新开始判断下一个1s板卡的丢帧率
								g_permit_card_lost_frame[can_line][card_num] = 0;
							}
							else if((g_permit_card_lost_frame[can_line][card_num] >= 30) && (count_1s < 50))
							{
                ...
                if (1 == p_can_buff[can_line]->DIO_DI[card_num].data[offset].frame_send_flag)
                {
                    ...
                    else
                    {
                        ...
                        if (time_after > FRAME_TIMEOUT)
                        {
							...
							//记录同一个板卡不同序号超时回复的个数  add by 3.8
							g_permit_card_lost_frame[can_line][card_num]++;
							p_can_buff[can_line]->DIO_DI[card_num].data[offset].frame_send_flag = 0;
							p_can_buff[can_line]->DIO_DI[card_num].data[offset].frame_recv_flag = 0;
							
							sprintf(str, "%d %d %d %d", can_line, offset, card_num, g_permit_card_lost_frame[can_line][card_num]);
							save_log(1, __LINE__, str);
							
							//20ms发送30帧，1s发送1500帧，丢帧率为10%，单个板卡最大允许丢帧数为15
							if((g_permit_card_lost_frame[can_line][card_num] < 30) && (count_1s >= 50))
							{
								count_1s = 0;   //1s内计时清零，重新开始判断下一个1s板卡的丢帧率
								g_permit_card_lost_frame[can_line][card_num] = 0;
                ...
                if (1 == p_can_buff[can_line]->DIO_DO[card_num].data[offset].frame_send_flag)
                {
                    ...
                    else
                    {
                        ...
                        if (time_after > FRAME_TIMEOUT)
                        {
							...
							g_permit_card_lost_frame[can_line][card_num]++;
							p_can_buff[can_line]->DIO_DO[card_num].data[offset].frame_send_flag = 0;
							p_can_buff[can_line]->DIO_DO[card_num].data[offset].frame_recv_flag = 0;
...
int set_tcp_data(uint8 *p_buff)
{
    ...
    // 打包plc数据
	memcpy(g_plc_data+20, g_plc_data1, 10);  //将回采的DO数据填入后20字节
